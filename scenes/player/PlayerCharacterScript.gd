...
func _ready():
	...
	if right_hand_scene != null:
		$CameraHolder/Camera3D/rightHand.add_child(right_hand_scene.instantiate())
	if left_hand_scene != null:
		$CameraHolder/Camera3D/leftHand.add_child(left_hand_scene.instantiate())
...
func move(delta):
	#direction input
	inputDirection = Input.get_vector("left", "right", "forward", "back")
